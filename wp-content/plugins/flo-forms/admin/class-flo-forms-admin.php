...
if (!class_exists('Flo_Forms_Admin')) {
	class Flo_Forms_Admin {
		...
		public function enqueue_styles() {
			...
			if (
				isset($screen->post_type) && in_array($screen->post_type, $cpt) ||
...
?>
			...
			if ($query->post_count) {
				...
				?>
						<div class="notice notice-error is-dismissible">
							...
							<h4><?php echo sprintf(
										__('There are %d unread Flo Forms entries. Please %s check them %s', 'flo-forms'),
										$unread_count,
										'<a href="' . admin_url('edit.php?post_type=flo_form_entry') . '">',
										'</a>'
									); ?></h4>
		...
		public static function flo_update_site_url_notice() {
			...
			<div class="notice notice-warning is-dismissible">
				<p><?php echo sprintf(
							__('It looks like the site is using https, but the "Site URL" settings are still using http.
    ...
    themes or plugins functionality. Check the %s documentation %s for details.', 'flo-forms'),
							'<a href="https://docs.flothemes.com/flothemes-flo-forms/#the-form-stopped-working-what-can-i-do" target="_blank" >',
							'</a>'
						);
						?></p>
			</div>
		...
		?>
			<div class="notice notice-error ff-email-issue-notice" style="position: relative;">
				...
				<p>
					<?php echo sprintf(
						__('Depending on the hosting server configuration and a combination of other factors there may be issues with <u><b>sending</b> and <b>receiving</b></u> emails.
  ...
  Also we strongly recommend testing the contact form on regular basis, and especially after updates to the site or plugins are made.', 'flo-forms'),
						'<a href="' . admin_url('admin.php?page=flo_forms_settings&tab=test-email') . '">',
						'</a>',
						'<a href="https://help.flothemes.com/article/533-troubleshooting-floforms-issues" target="_blank">',
						'</a>',
						'<a href="' . admin_url('edit.php?post_type=flo_form_entry') . '">',
						'</a>'
					); ?>
